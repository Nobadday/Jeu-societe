...
/// <summary>
/// 
/// Draw all the map layers before the given name (excluded) 
/// 
/// </summary>
void Tiled::DrawMapLayers(sf::RenderWindow& _window, const sf::Vector2f& _camera,std::string _name)
{
	for (const auto& layer : mapLayers)
	{
...
/// <summary>
/// 
/// Draw all the map layers after the given name (include) 
/// 
/// </summary>
void Tiled::DrawMapLayers(sf::RenderWindow& _window, const sf::Vector2f& _camera,const char * _name)
{
	bool draw = false;
	for (const auto& layer : mapLayers)
	{
		if (layer.GetName() == _name or draw)
		{
			draw = true;
			layer.Draw(_window, tilesetManager, _camera);
		}
