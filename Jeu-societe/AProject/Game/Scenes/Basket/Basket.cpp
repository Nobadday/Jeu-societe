...
void Basket::Load(void)
{
	...
	// Cr�er les joueurs bas�s sur m_gonnaPlayIndex
	for (size_t i = 0; i < m_data->gameData->m_gonnaPlayIndex.size(); ++i)
	{
		...
		player.m_won = false;
		player.m_winOrder = -1;  // NOUVEAU: -1 = n'a pas encore gagn�
...
void Basket::PollEvent(sf::Event& _event)
{
	...
	else if (m_data->state == STATE_GAMEOVER)
	{
		if (_event.type == sf::Event::JoystickButtonPressed)
		{
			...
			{
				// Retour au Board avec les r�sultats
...
void Basket::Update(float _deltaTime)
{
	...
	if (m_data->state == STATE_PLAYING)
	{
		...
		if (m_data->timer <= 0.f)
		{
			...
			// Trier les joueurs par ordre de victoire et mettre � jour m_winIndex
			m_data->gameData->m_winIndex.clear();
			
			// Cr�er un vecteur de paires (winOrder, playerID) pour trier
			...
			for (BasketPlayer& player : m_data->players)
			{
				...
				{
					// Les perdants ont un winOrder tr�s �lev� (fin de liste)
					winOrderList.push_back({9999, player.m_id});
				}
			}
			
			// Trier par ordre de victoire (1er, 2�me, 3�me, puis perdants)
		...
		for (size_t i = 0; i < m_data->players.size(); ++i)
		{
			...
			// V�rifier si le joueur participe
