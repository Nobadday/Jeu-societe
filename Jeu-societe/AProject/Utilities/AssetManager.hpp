...
class AssetManager
{
	...
	private:
		// Adds an empty asset container
		int CreateContainer(const std::string& _name);
		
		void AddAsset(const std::string& _name, void* _object, AssetType _type, int _containerID);
		void AddAsset(const std::string& _name, void* _object, AssetType _type, const std::string& _container);
	...
	public:
		...
		// Unloads a manifest
		bool UnloadContainer(int _index);
		bool UnloadContainer(const std::string& _name);
