...
TextureWhat::TextureWhat(void) : sf::Texture(), AtlasContainer()
{
...
bool TextureWhat::LoadFromFilePicture(const sf::String& _filePath, bool _resetAtlas, const sf::IntRect& _area)
{
...
bool TextureWhat::LoadFromFilePicture(const sf::Image& _image, bool _resetAtlas, const sf::IntRect& _area)
{
...
bool TextureWhat::LoadFromFile(std::fstream& _file, const std::string& _dirPath, TextureWhat::ParseType _parseType)
{
	if (_file.fail())
	{
		return false;
	...
	{
		printf("[WARNING] TextureAtlas : Couldn't load the atlas's picture at \"%s\". Atlas Data is still loaded.\n", textureName.c_str());
