...
void ArmWrestling::Update(float _deltaTime)
{
	...
	switch (m_state)
	{
		case ArmWrestling::GAME:
			if (m_playerArms.getScale().x != 0.f && m_playerArms.getScale().y != 0.f)
			{
				m_playerArms.setScale(Lerp2D(m_playerArms.getScale(), sf::Vector2f(1.2f, 1.2f),_deltaTime*10.f));
			}
			char buffer[50];
...
void ArmWrestling::PopTexture(void)
{
	//m_playerArms.setScale(1.2f, 1.2f);

	m_playerArms.setScale(1.5f, 1.5f);
}